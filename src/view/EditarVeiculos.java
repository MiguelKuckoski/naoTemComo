			...
			btnExcluir.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent arg0) {
					...
					if (getTable().getSelectedRowCount() == 1) {
						...
						if (veiculo == null) {
							for (Veiculo veiculo2 : controle.getLoggedUser().getVeiculos()) {
								if (veiculo2.getPlaca() == placa) {
									veiculo = veiculo2;
									break;
								}
							}
							controle.getLoggedUser().getVeiculos().remove(veiculo);
						} else {
		...
		if (btnConfirmar == null) {
			...
			btnConfirmar.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					controle.getLoggedUser().getVeiculos().addAll(getVeiculosNovos());
					getVeiculosNovos().clear();
